...
def init_db():
    ...
    def columns(table):
        rows = x.execute("""SELECT column_name FROM information_schema.columns
    ...
    for table, fields in migrations.items():
        ...
        for name, definition in fields.items():
            if name not in have:
                x.execute(f"ALTER TABLE {table} ADD COLUMN {name} {definition}")
    ...
    if not x.execute("SELECT id FROM users WHERE username='parto'").fetchone():
    ...
    if not x.execute("SELECT id FROM rooms WHERE username='parto'").fetchone():
        x.execute("""INSERT INTO rooms(name,username,kind,owner,emoji,bio)
            VALUES(%s,%s,'channel','parto',%s,%s)""",
            ("پرتو","parto","⚡","کانال رسمی پرتو"))
    x.commit()
    x.close()

def is_pro(username):
    x = db()
    row = x.execute("SELECT pro FROM users WHERE username=%s", (username,)).fetchone()
    x.close()
...
def me():
    ...
    if not name:
        return None
    x = db()
    u = x.execute("SELECT * FROM users WHERE username=%s", (name,)).fetchone()
    x.close()
    return u
...
def register():
    ...
    x=db()
    try:
        x.execute("INSERT INTO users(username,email,password) VALUES(%s,%s,%s)",
                  (request.form["username"].strip(),request.form["email"].strip(),generate_password_hash(request.form["password"])))
        x.commit()
    except psycopg2.IntegrityError:
        x.rollback();x.close();return "Username or email already exists."
    name=request.form["username"].strip();x.close();session["user"]=name;return redirect("/chat")

@app.route("/login",methods=["POST"])
def login():
    x=db();u=x.execute("SELECT * FROM users WHERE email=%s",(request.form["email"].strip(),)).fetchone();x.close()
    if not u or not check_password_hash(u["password"],request.form["password"]): return "Invalid login."
    if u["banned"]: return "Account is banned."
...
def create():
    ...
    x=db()
    try:
        x.execute("""INSERT INTO rooms(name,username,kind,owner,emoji,bio) VALUES(%s,%s,%s,%s,%s,%s)""",
                  (request.form["name"][:80],request.form["username"][:40],request.form["kind"],u["username"],
                   request.form.get("emoji","💬")[:8],request.form.get("bio","")[:200]))
        x.commit()
    except psycopg2.IntegrityError:
        x.rollback();x.close();return "This ID is already in use."
    x.close()
...
def pro_page():
    ...
    if not u["pro"]:
        ...
        <div class="box">
            ...
            <a href="/pro">PRO</a><a href="/profile/%s">بازگشت به پروفایل</a>
        </div>
        """ % esc(u["username"]))
    return page("""
    <div class="box">
