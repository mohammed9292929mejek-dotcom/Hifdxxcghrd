...
def search():
    ...
    if q:
        x.execute("SELECT * FROM rooms WHERE (public=1 OR owner=%s) AND (username ILIKE %s OR name ILIKE %s) ORDER BY public DESC LIMIT 50",(u['username'],'%'+q+'%','%'+q+'%'));rooms=x.fetchall()
