...
def layout(title, body, active="chat"):
    ...
    links=[("chat","⌂","خانه","/chat"),("messages","✉","دایرکت","/messages"),("search","⌕","جستجو","/search"),
           ("create","＋","پست","/chat#create"),("profile","◉","پروفایل","/profile")]
...
def chat():
    ...
    x=db()
    posts=x.execute("""SELECT m.*, COALESCE(r.name,u2.username) AS author_name,
        ...
        WHERE m.room IS NULL OR r.kind='channel'
        ORDER BY m.id DESC LIMIT 100""",(u['username'],)).fetchall()
    today=datetime.now().date().isoformat()
    today_count=x.execute("SELECT COUNT(*) AS c FROM messages WHERE username=%s AND created_at LIKE %s",(u['username'],today+'%')).fetchone()['c']
    x.close()
    limit=1000 if u['pro'] else (5 if u['verified'] else 3)
    composer=f"""<div class="card" id="create"><form method="post" action="/post/create" enctype="multipart/form-data">
    <textarea name="text" maxlength="2000" placeholder="چه خبر؟" required></textarea>
    <div class="sub">امروز {today_count}/{limit} پست</div>
    <input type="file" name="media" accept="image/*,video/*,audio/*,.pdf,.zip,.txt,.doc,.docx">
    <button>پست کردن</button></form></div>"""
    out=''
    for m in posts:
        badge=" <span class='badge'>✓</span>" if m['author_verified'] else ''
        pro=" <span class='badge pro'>PRO</span>" if m['author_pro'] else ''
        av=avatar_html({'avatar':m['author_avatar'],'emoji':m['author_emoji']}) if m['author_avatar'] or m['author_emoji'] else '<div class="avatar">👤</div>'
        if m.get('room'):
            pi=page_image_url(m['author_username'])
            if pi: av=f"<img class='avatar' src='{pi}' alt=''>"
        media=''
        ...
        elif mu and mt.startswith('video/'): media=f"<video controls style='max-width:100%'><source src='/uploads/chat/{esc(mu)}' type='{esc(mt)}'></video>"
        elif mu and mt.startswith('audio/'): media=f"<audio controls src='/uploads/chat/{esc(mu)}'></audio>"
        elif mu: media=f"<a href='/uploads/chat/{esc(mu)}' download>📎 {esc(m.get('media_name') or 'فایل')}</a>"
        owner_actions=''
        if m['username']==u['username'] or is_admin(u): owner_actions=f" <a href='/post/edit/{m['id']}'>ویرایش</a> <form style='display:inline' method='post' action='/post/delete/{m['id']}'><button class='btn-sm warn'>حذف</button></form>"
        like_label='♥' if m['liked'] else '♡'
        out+=f"""<article class="card" style="padding:14px"><div style="display:flex;gap:10px">{av}<div><a href="/profile/{esc(m['author_username'])}"><b>{esc(m['author_name'])}</b>{badge}{pro}</a><div class="sub">@{esc(m['author_username'])}</div></div></div>
        <div style="margin:10px 0;line-height:1.8">{esc(m['text'])}</div>{media}<div class="post-actions"><form style="display:inline" method="post" action="/post/like/{m['id']}"><button class="btn-sm">{like_label} {m['likes']}</button></form> <a class="btn-sm" href="/post/{m['id']}/comments">💬 {m['comments']}</a>{owner_actions}</div></article>"""
    body=f"<div class='page'><h2>خانه</h2>{composer}<div class='list'>{out or '<p class=\"sub\">هنوز پستی نیست.</p>'}</div></div>"
...
def profile(username):
    u=me()
    if not u:return redirect("/")
    x=db();z=x.execute("SELECT * FROM users WHERE username=%s",(username,)).fetchone(); followers=x.execute("SELECT COUNT(*) AS c FROM follows WHERE target=%s",(username,)).fetchone()['c']; following=x.execute("SELECT COUNT(*) AS c FROM follows WHERE follower=%s",(username,)).fetchone()['c']; following_me=bool(x.execute("SELECT id FROM follows WHERE follower=%s AND target=%s",(u['username'],username)).fetchone()) if u['username']!=username else False; posts=x.execute("SELECT * FROM messages WHERE username=%s ORDER BY id DESC LIMIT 30",(username,)).fetchall(); x.close()
    if not z:return "User not found."
    badges=("<span class='badge'>✓</span>" if z["verified"] else "")+("<span class='badge pro'>PRO</span>" if z["pro"] else "")
    button=f"<a href='/editprofile'>ویرایش پروفایل</a>" if u["username"]==username else f"<a href='/private/{esc(username)}'>دایرکت</a> <form style='display:inline' method='post' action='/follow/{esc(username)}'><button class='btn-sm'>{'دنبال نکردن' if following_me else 'دنبال کردن'}</button></form>"
    big_avatar = avatar_html(z, cls="bigavatar")
    body=f"""<div class='page'><div class='profile'><div class='cover'></div>{big_avatar}
    <h2>@{esc(z["username"])} {badges}</h2><p class='sub'>{esc(z["bio"] or "No bio yet.")}</p><p class='sub'>{followers} دنبال‌کننده · {following} دنبال‌شونده</p><div class='actions'>{button}{" <a href='/logout'>خروج از حساب</a>" if u['username']==username else ''}</div></div></div>"""
...
def admin_panel():
    ...
    post_rows = "".join(f"<div class='admin-row'><div class='info'><div class='name'>#{p['id']} · @{esc(p['username'])}</div><div class='sub'>{esc(p['text'][:180])}</div></div><div class='admin-actions'><a class='btn-sm' href='/post/edit/{p['id']}'>ویرایش</a><form method='post' action='/post/delete/{p['id']}'><button class='btn-sm warn'>حذف</button></form></div></div>" for p in posts)
