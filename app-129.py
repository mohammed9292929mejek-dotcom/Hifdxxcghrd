...
def init_db():
    ...
    try:
        ...
        x.execute("""CREATE TABLE IF NOT EXISTS rooms(
          ...
          verified INTEGER DEFAULT 0, members INTEGER DEFAULT 0, public INTEGER DEFAULT 0)""")
        ...
        migrations={
          ...
          'rooms': {'verified':'INTEGER DEFAULT 0','members':'INTEGER DEFAULT 0','public':'INTEGER DEFAULT 0','bio':"TEXT DEFAULT ''"},
...
def chat():
    ...
    if r:return r
    u=current();x=db()
    x.execute("SELECT * FROM rooms WHERE public=1 ORDER BY CASE WHEN username='parto' THEN 0 WHEN username='gangs' THEN 1 ELSE 2 END,id")
    public=x.fetchall()
...
def search():
    ...
    if q:
        x.execute("SELECT * FROM rooms WHERE username ILIKE %s OR name ILIKE %s ORDER BY public DESC LIMIT 50",('%'+q+'%','%'+q+'%'));rooms=x.fetchall()
...
def join():
    ...
    if r:return r
    if request.method=='POST':
        val=request.form.get('link','').strip().rstrip('/'); user=val.split('/')[-1].lstrip('@')
        return redirect('/c/'+user)
    body='''<div class="card hero"><h1 class="title">Join by link</h1><p class="muted">Paste a Parto channel or group link/username.</p><form class="form" method="post"><input class="input" name="link" placeholder="parto.app/c/username or @username" required><button class="btn primary">Open</button></form></div>''';return layout(body,'Join')
...
def room(username):
    ...
    can_post=room['kind']=='group' or room['owner']==u['username']
    form=''
    if can_post:
    ...
    else: form='<div class="composer"><span class="muted">Only the channel owner can post here.</span></div>'
    body=f'''<div class="card chatbox"><div class="chathead row between"><div class="row"><div class="avatar">{esc(room['emoji'])}</div><div><div class="userline">{esc(room['name'])} {badge(room)}</div><div class="muted">@{esc(room['username'])} · {int(room['members'] or 0)} members</div></div></div><a class="btn ghost" href="/edit-room/{room['id']}">Edit</a></div><div class="messages">{bubbles or '<div class="empty">No messages yet.</div>'}</div>{form}</div>''';return layout(body,room['name'],'home')
...
def send_room(room_id):
    ...
    if room['kind']=='channel' and room['owner']!=u['username'] and not is_admin(u):x.close();return redirect('/c/'+room['username'])
...
def admin():
    ...
    x=db();x.execute('SELECT * FROM users ORDER BY id DESC LIMIT 100');users=x.fetchall();x.execute('SELECT * FROM rooms ORDER BY id DESC LIMIT 100');rooms=x.fetchall();x.close()
    us=''.join(f'''<div class="item row between"><div><b>@{esc(z['username'])}</b> {badge(z)} {('<span class="pro">PRO</span>' if z['pro'] else '')}<div class="muted">banned: {z['banned']}</div></div><div class="row"><form method="post" action="/admin/toggle"><input type="hidden" name="kind" value="user"><input type="hidden" name="id" value="{z['id']}"><button class="btn ghost">Verify/Unverify</button></form><form method="post" action="/admin/pro"><input type="hidden" name="id" value="{z['id']}"><button class="btn ghost">PRO</button></form></div></div>''' for z in users)
    rs=''.join(f'''<div class="item"><b>{esc(z['name'])}</b> <span class="muted">@{esc(z['username'])}</span><div class="muted">owner: @{esc(z['owner'])} · public: {z['public']}</div><form method="post" action="/admin/room"><input type="hidden" name="id" value="{z['id']}"><button class="btn ghost">Toggle verified</button></form></div>''' for z in rooms)
    body=f'''<div class="card"><div class="hero"><h1 class="title">Admin</h1><p class="muted">Moderation and verification.</p></div><div class="list">{us}</div></div><div class="card" style="margin-top:12px"><div class="hero"><h2>Spaces</h2></div><div class="list">{rs}</div></div>''';return layout(body,'Admin','settings')

@app.post('/admin/toggle')
def admin_toggle():
    u=current()
    if not is_admin(u):return redirect('/chat')
    x=db();x.execute('SELECT verified FROM users WHERE id=%s',(request.form.get('id'),));z=x.fetchone();x.execute('UPDATE users SET verified=%s WHERE id=%s',(0 if z and z['verified'] else 1,request.form.get('id')));x.commit();x.close();return redirect('/admin')
@app.post('/admin/pro')
def admin_pro():
    u=current()
    if not is_admin(u):return redirect('/chat')
    x=db();x.execute('SELECT pro FROM users WHERE id=%s',(request.form.get('id'),));z=x.fetchone();x.execute('UPDATE users SET pro=%s WHERE id=%s',(0 if z and z['pro'] else 1,request.form.get('id')));x.commit();x.close();return redirect('/admin')
@app.post('/admin/room')
def admin_room():
    u=current()
    if not is_admin(u):return redirect('/chat')
    x=db();x.execute('SELECT verified FROM rooms WHERE id=%s',(request.form.get('id'),));z=x.fetchone();x.execute('UPDATE rooms SET verified=%s WHERE id=%s',(0 if z and z['verified'] else 1,request.form.get('id')));x.commit();x.close();return redirect('/admin')
