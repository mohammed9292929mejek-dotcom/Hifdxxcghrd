...
.admin-row{display:flex;align-items:center;gap:14px;padding:15px;border:1px solid #1d2940;background:#0e1524e8;border-radius:21px;flex-wrap:wrap}
.admin-row .info{flex:1;min-width:140px}.admin-actions{display:flex;gap:6px;flex-wrap:wrap}
.btn-sm{width:auto;padding:8px 12px;margin:0;font-size:12px;border-radius:10px;background:#182238}
.btn-sm.warn{background:linear-gradient(135deg,#ff4d4d,#c92a2a)}.btn-sm.on{background:linear-gradient(135deg,#765cff,#477cff)}
...
def layout(title, body, active="chat"):
    ...
    <nav class='bottom'>{bottom}{admin_bottom}</nav></body></html>"""
...
def chat():
    ...
    for r in other_rooms:
        ...
        cards+=f"""<a class='item' href='/room/{r["id"]}'><div class='avatar'>{esc(r["emoji"])}</div><div class='info'>
        <div class='name'>{esc(r["name"])}</div><div class='sub'>@{esc(r["username"])} · {kind_label}</div></div><div class='arrow'>‹</div></a>"""
...
def room(rid):
    ...
    if can_send:
        composer = f"<form class='composer' method='post' action='/send/{rid}'><input name='text' placeholder='پیام...' required><button>➤</button></form>"
    else:
...
def admin_panel():
    ...
    for z in users:
        ...
        <div class='avatar'>{esc(z["emoji"])}</div>
    ...
    body = f"<div class='page'><h2>پنل مدیریت</h2><div class='list'>{rows}</div></div>"
