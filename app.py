...
def layout(title, body, active="chat"):
    ...
    links=[("chat","⌂","خانه","/chat"),("messages","💬","پیام‌ها","/messages"),("search","⌕","جستجو","/search"),
...
def chat():
    ...
    x=db()
    # Legacy kind='channel' is intentionally kept in the database, but is now
    # presented as a Twitter-like Page. No existing rows are deleted or renamed.
    pages=x.execute("SELECT * FROM rooms WHERE kind='channel' ORDER BY id DESC").fetchall()
    posts=x.execute("""SELECT m.*,r.name AS page_name,r.username AS page_username,r.emoji AS page_emoji,r.verified AS page_verified
        FROM messages m JOIN rooms r ON r.id=m.room
        WHERE r.kind='channel' ORDER BY m.id DESC LIMIT 100""").fetchall()
    x.close()
    out=""
    for m in posts:
        image=f"<img class='msg-img' src='/uploads/chat/{esc(m['image'])}' alt=''>" if m.get('image') else ""
        badge=" <span class='badge'>✓</span>" if m['page_verified'] else ""
        out+=f"""<article class='card' style='padding:16px'>
        <a href='/room/{m['room']}'><div class='name'>@{esc(m['page_username'])}{badge}</div><div class='sub'>{esc(m['page_name'])}</div></a>
        <div style='margin-top:10px;line-height:1.8'>{image}{esc(m['text'])}</div>
        </article>"""
    page_links="".join(f"<a class='item' href='/room/{r['id']}'><div class='avatar'>{esc(r['emoji'])}</div><div class='info'><div class='name'>{esc(r['name'])} {'<span class=\'badge\'>✓</span>' if r['verified'] else ''}</div><div class='sub'>@{esc(r['username'])} · پیج</div></div><div class='arrow'>‹</div></a>""" for r in pages[:20])
    body=f"""<div class='page'><h2>خانه</h2><div class='list'>{out or '<p class=\"sub\">هنوز پستی نیست.</p>'}</div>
    <h3 class='admin-section-title'>پیج‌ها</h3><div class='list'>{page_links or '<p class=\"sub\">هنوز پیجی ساخته نشده.</p>'}</div></div>"""
...
def messages():
    ...
    groups=x.execute("SELECT * FROM rooms WHERE kind='group' ORDER BY id DESC").fetchall();x.close()
    cards=""
    for z in private_users:
        unread=f"<span class='badge pro'>{int(z['unread'])}</span>" if z['unread'] else ''
        cards+=f"<a class='item' href='/private/{esc(z['username'])}'>{avatar_html(z)}<div class='info'><div class='name'>@{esc(z['username'])} {unread}</div><div class='sub'>پیام خصوصی</div></div><div class='arrow'>‹</div></a>"
    for g in groups:
        cards+=f"<a class='item' href='/room/{g['id']}'><div class='avatar'>{esc(g['emoji'])}</div><div class='info'><div class='name'>{esc(g['name'])}</div><div class='sub'>@{esc(g['username'])} · گروه</div></div><div class='arrow'>‹</div></a>"
    return layout("پیام‌ها",f"<div class='page'><div class='list'>{cards or '<p class=\"sub\">هنوز پیامی نداری.</p>'}</div></div>","messages")
...
def room(rid):
    ...
    page_upload=(f"<form method='post' action='/page/{rid}/image' enctype='multipart/form-data'><label class='btn-sm on upload-label'>عکس پیج<input type='file' name='image' accept='image/*' hidden onchange='this.form.submit()'></label></form>" if r["kind"]=="channel" and r["owner"]==u["username"] else "")
...
def send(rid):
    ...
    x=db();r=x.execute("SELECT * FROM rooms WHERE id=%s",(rid,)).fetchone()
    if not r:x.close();return "Chat not found."
    if r["kind"]=="channel" and r["owner"]!=u["username"]:x.close();return "Only the page owner can post.",403
...
def create():
    ...
    if not u:return redirect("/")
    if request.method=="GET":
        return layout("ساخت",f"""<div class='page'><div class='box'><h2>ساخت گروه / پیج</h2>
