...
def init_db():
    ...
    try:
        con.execute("ALTER TABLE users ADD COLUMN emoji TEXT NOT NULL DEFAULT '👤'")
    except sqlite3.OperationalError:
        pass

    try:
        con.execute("ALTER TABLE chats ADD COLUMN emoji TEXT NOT NULL DEFAULT '💬'")
    except sqlite3.OperationalError:
        pass
    ...
    # Create the official Parto channel once.
    parto = con.execute(
        "SELECT id FROM chats WHERE username='parto' AND kind='channel'"
    ).fetchone()

    if not parto:
        owner = con.execute(
            "SELECT id FROM users WHERE username=?",
            (os.environ.get("PARTO_ADMIN_USER", "admin"),)
        ).fetchone()

        if owner:
            con.execute(
                """INSERT INTO chats
                (kind,title,username,emoji,owner_id,created_at)
                VALUES(?,?,?,?,?,?)""",
                ("channel", "پرتو", "parto", "⚡", owner["id"], datetime.now().isoformat())
            )

            parto_id = con.execute(
...
def emoji_picker(selected="😎", name="emoji"):
    html = f'<div style="display:flex;flex-wrap:wrap;gap:6px;margin:8px 0">'
    for emoji in PROFILE_EMOJIS:
        checked = "checked" if emoji == selected else ""
        html += (
            f'<label style="cursor:pointer">'
            f'<input type="radio" name="{name}" value="{emoji}" {checked} '
            f'style="display:none">'
            f'<span style="font-size:28px;padding:5px;display:inline-block;'
            f'border-radius:9px;background:#2b3945">{emoji}</span>'
            f'</label>'
        )
    html += "</div>"
    return html
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            # Automatically join the official Parto channel.
            parto = con.execute(
                "SELECT id FROM chats WHERE username='parto' AND kind='channel'"
            ).fetchone()
...
def create_space(kind):
    ...
    emoji = request.form.get("emoji", "👥" if kind == "group" else "📢")
    if emoji not in PROFILE_EMOJIS:
        emoji = "👥" if kind == "group" else "📢"
...
def chat(chat_id):
    ...
    if request.method == "POST":
        # In channels, only the channel owner can publish messages.
        if chat_row["kind"] == "channel" and chat_row["owner_id"] != user["id"]:
            con.close()
            return "فقط مالک کانال می‌تواند پیام ارسال کند.", 403
        ...
        if text or file_name:
            con.execute(
                ...
                (chat_id,sender_id,text,file_name,file_type,reply_to,created_at)
                VALUES(?,?,?,?,?,?,?)""",
                (
                    chat_id,
                    user["id"],
    ...
    messages = con.execute(
        ...
        SELECT
            ...
            ru.display_name AS reply_name,
            ru.emoji AS reply_emoji
        FROM messages m
    ...
    for message in messages:
        ...
        sender_icon = message["emoji"] or "👤"

        body += f"""
        <div class="{css}">
            <b>
                {sender_icon} {message['display_name']}
                {badges(message)}
            </b>
        """

        if message["reply_to"]:
            reply_text = message["reply_text"] or "پیام رسانه‌ای"
            reply_name = message["reply_name"] or "کاربر"
            reply_emoji = message["reply_emoji"] or "👤"
            body += f"""
            <div style="border-right:3px solid #2aabee;
                        background:#202b36;
                        padding:6px 9px;
                        margin:6px 0;
                        border-radius:8px;
                        font-size:12px">
                {reply_emoji} <b>{reply_name}</b><br>
                <span style="color:#b8c3cc">{reply_text[:180]}</span>
            </div>
            """
        ...
        body += f"""
            ...
            <button type="button"
                    ...
                    style="background:none;border:0;color:#2aabee;
    ...
    if chat_row["kind"] == "channel" and chat_row["owner_id"] != user["id"]:
        ...
        <div class="box center" style="margin-bottom:0">
            📢 فقط مالک کانال می‌تواند پیام ارسال کند.
        </div>
        """
    else:
        body += """
    ...
    <script>
    function replyTo(id) {
        const field = document.getElementById("reply_to");
        const text = document.getElementById("message_text");
        if (field) field.value = id;
        if (text) {
            text.placeholder = "↩ پاسخ به پیام...";
            text.focus();
        }
...
def profile():
    ...
    if request.method == "POST":
        name = request.form.get("name", user["display_name"]).strip()
        emoji = request.form.get("emoji", user["emoji"]).strip() or "👤"
        bio = request.form.get("bio", user["bio"]).strip()[:160]

        con = db()

        # Normal accounts cannot upload profile photos.
        if user["is_pro"]:
            ...
            con.execute(
                """UPDATE users
                SET display_name=?, avatar=?, emoji=?, bio=?
                WHERE id=?""",
                (name, filename, emoji, bio, user["id"])
            )
        else:
            # Pro-only photo: normal users are forced to emoji.
            con.execute(
                """UPDATE users
                SET display_name=?, avatar=NULL, emoji=?, bio=?
                WHERE id=?""",
                (name, emoji, bio, user["id"])
            )
    ...
    if user["is_pro"] and user["avatar"]:
        avatar_html = f'<img src="/uploads/{user["avatar"]}">'
    else:
        avatar_html = user["emoji"] or "👤"

    pro_note = (
        "🔴 حساب Pro: امکان عکس پروفایل فعال است."
        if user["is_pro"]
        else
        "🙂 حساب عادی: از ایموجی به‌عنوان عکس پروفایل استفاده کن."
    )
    ...
    <div class="box center">
        <div class="avatar"
             style="margin:auto;width:90px;height:90px;font-size:45px">
            {avatar_html}
        </div>
        ...
        <a class="btn" href="{url_for('start_chat', other_id=user['id'])}"
           onclick="event.preventDefault(); document.getElementById('private-chat-form').submit();">
            💬 چت خصوصی
        </a>

        <form id="private-chat-form" method="post"
              action="{url_for('start_chat', other_id=user['id'])}" style="display:none">
        </form>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <textarea name="bio"
                      maxlength="160"
                      placeholder="بیوگرافی">{user['bio'] or ''}</textarea>

            <label>ایموجی پروفایل</label>
            {emoji_picker(user["emoji"] or "😎")}
